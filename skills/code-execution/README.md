...
Code runs in a minimal sandboxed interpreter with no file or network access. The sandbox exposes `await llm(prompt)` for LLM reasoning (classify, summarize, extract). See the SKILL.md for full sandbox limitations.
